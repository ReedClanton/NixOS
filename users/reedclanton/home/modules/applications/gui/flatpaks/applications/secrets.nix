{ config, lib, user, ... }: {
  ...
  home.activation.linkKeePassFile = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ln -sf ${config.xdg.configHome}/NixOS/users/${user.name}/home/config/kee-pass/reedclanton.kdbx ${config.xdg.userDirs.documents}/Secrets/KeePass/reedclanton.kdbx
  '';
